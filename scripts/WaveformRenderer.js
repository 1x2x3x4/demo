/**
 * WaveformRenderer.js
 * 波形渲染器类 - 提供高级波形绘制和控制功能
 * 
 * 该类合并了WaveDrawer.js和WaveformRenderer.js的功能，
 * 提供了更完整的波形渲染、触发控制、校准和动画效果。
 * 
 * @author 波形模拟系统团队
 * @version 2.0
 */
...
// 保留WaveDrawer命名空间以保持向后兼容
window.WaveDrawer = (function() {
  // 创建代理对象，将所有调用转发到WaveformRenderer静态方法
  let renderer = null;
  ...
  return {
    ...
    drawWave: function(ctx, params) {
      const r = getRenderer();
      r.ctx = ctx;
      // 将旧格式参数转换为新格式
      r.signalParams.type = params.signalType || 'sine';
      r.signalParams.frequency[params.line] = params.frequencies[params.line] || 1;
      r.signalParams.amplitude[params.line] = (params.peakValues[params.line] || 2) / 2;
      r.signalParams.phase = params.phase || 0;
      r.signalParams.phaseDiff = params.phaseDiff || 0;
      r.viewport.timeDiv = params.timeDiv || 1;
      r.viewport.voltsDiv = params.voltsDiv || { 1: 1, 2: 1 };
      r.viewport.horizontalPosition = params.horizontalPosition || 0;
      r.viewport.verticalPosition[params.line] = params.verticalPosition || 0;
      r.calibration.displayAdjustFactors = params.displayAdjustFactors || { time: 1.0, volts: { 1: 1.0, 2: 1.0 } };
      r.calibration.factor = params.calibrationFactor || 1.0;
      
      // 修改颜色
      r.config.colors[params.line === 1 ? 'channel1' : 'channel2'] = params.color || (params.line === 1 ? '#2196F3' : '#FF5722');
      
      return r.drawChannelWave(params.line);
    },
    drawTriggerLevel: function(ctx, triggerLevelPosition) {
      const r = getRenderer();
      r.ctx = ctx;
      // 反向计算触发电平
      const height = ctx.canvas.height || 400;
      r.config.trigger.level = ((height / 2) - triggerLevelPosition) / 40;
      return r.drawTriggerLevel();
    },
    drawOverlayWave: function(ctx, params) {
      const r = getRenderer();
      r.ctx = ctx;
      // 更新参数
      r.signalParams.type = params.signalType || 'sine';
      r.signalParams.frequency = params.frequencies || { 1: 1, 2: 1 };
      r.signalParams.amplitude = {};
      r.signalParams.amplitude[1] = (params.peakValues[1] || 2) / 2;
      r.signalParams.amplitude[2] = (params.peakValues[2] || 2) / 2;
      r.signalParams.phase = params.phase || 0;
      r.signalParams.phaseDiff = params.phaseDiff || 0;
      r.viewport.timeDiv = params.timeDiv || 1;
      r.viewport.voltsDiv = params.voltsDiv || { 1: 1, 2: 1 };
      r.viewport.horizontalPosition = params.horizontalPosition || 0;
      r.viewport.verticalPosition = params.verticalPosition || { 1: 0, 2: 0 };
      r.calibration.displayAdjustFactors = params.displayAdjustFactors || { time: 1.0, volts: { 1: 1.0, 2: 1.0 } };
      r.calibration.factor = params.calibrationFactor || 1.0;
      r.config.channels = { 1: { active: params.inputActive[1] }, 2: { active: params.inputActive[2] } };
      
      return r.drawOverlayWaves();
    },
    adjustPhaseForTrigger: function(params) {
      // 保持原有功能原样返回
      return params.phase;
    },
    // 其他兼容函数
    calculateVoltage: function(waveType, phaseVal, amplitude) {
      return getRenderer().calculateVoltage(waveType, phaseVal, amplitude);
    },
    updateWaveformType: function(renderParams, waveType) {
      return Object.assign({}, renderParams, { signalType: waveType, needsRedraw: true });
    },
    resetTriggerSystem: function(triggerSettings) {
      return Object.assign({}, triggerSettings, {
        triggerLevel: 0,
        triggerMode: 'auto',
        triggerSlope: 'rising'
      });
    },
    toggleTriggerSlope: function(currentSlope) {
      return currentSlope === 'rising' ? 'falling' : 'rising';
    },
    adjustScopeSettings: function(settings, paramType, step, channel) {
      const result = Object.assign({}, settings);
      
      switch (paramType) {
        case 'timeDiv':
          result.timeDiv = Math.min(100, Math.max(0.1, settings.timeDiv + step));
          result.timeDiv = Number(result.timeDiv.toFixed(1));
          break;
        case 'voltsDiv':
          if (channel) {
            result.voltsDiv = Object.assign({}, settings.voltsDiv);
            result.voltsDiv[channel] = Math.min(10, Math.max(0.1, settings.voltsDiv[channel] + step));
    ...
    validateInputSettings: function(settings, type, channel) {
      const result = Object.assign({}, settings);
      
      if (type === 'time') {
        result.timeDiv = Math.min(100, Math.max(0.1, settings.timeDiv));
        result.timeDiv = Number(result.timeDiv.toFixed(1));
        ...
        if (isNaN(settings.voltsDiv[channel]) || settings.voltsDiv[channel] === null) {
          ...
          result.voltsDiv = Object.assign({}, settings.voltsDiv);
          result.voltsDiv[channel] = Math.min(10, Math.max(0.1, settings.voltsDiv[channel]));
          result.voltsDiv[channel] = Number(result.voltsDiv[channel].toFixed(2));
        }
      }
...
// 优化共享工具对象：添加更多通用功能，减少代码重复
window.WaveformUtilities = (function() {
  // 常量定义 - 统一管理常用常量
  const CONSTANTS = {
    CANVAS: {
      WIDTH: 800,
  ...
  function calculateVoltage(waveType, phaseVal, amplitude) {
    ...
    switch (waveType.toLowerCase()) {
      ...
      case 'triangle':
        return amplitude * (2 / Math.PI) * Math.asin(Math.sin(phaseVal));
      case 'sawtooth':
        return amplitude * (((phaseVal / twoPI) % 1) * 2 - 1);
      case 'pulse':
        return amplitude * ((phaseVal / twoPI % 1) < 0.2 ? 1 : -1);
      case 'noise':
  ...
  function drawGrid(ctx, options = {}) {
    const width = ctx.canvas.width || CONSTANTS.CANVAS.WIDTH;
    const height = ctx.canvas.height || CONSTANTS.CANVAS.HEIGHT;
    const gridColor = options.gridColor || '#2a2a2a';
    const axesColor = options.axesColor || '#666666';
    const lineWidth = options.lineWidth || 0.5;
    const gridSize = options.gridSize || CONSTANTS.GRID.SIZE;
    ...
    try {
      ...
      console.error('Grid drawing failed:', error);
    }
  }

  // 触发器相关工具函数
  function resetTriggerSystem(triggerConfig) {
    return {
      level: 0,
      mode: 'auto',
      slope: 'rising',
  ...
  // 计算有效校准因子（抽取重复逻辑）
  function calculateEffectiveFactor(useCalibration, calibrationFactor, displayAdjustFactors) {
    return useCalibration
      ? calibrationFactor * displayAdjustFactors.time
      : calibrationFactor * displayAdjustFactors.time;
  }
  
  // 计算有效电压分度值（抽取重复逻辑）
  function calculateEffectiveVoltsDiv(useCalibration, voltsDiv, line, calibrationFactor, displayAdjustFactors) {
    const voltAdjustFactor = useCalibration
      ? calibrationFactor * displayAdjustFactors.volts[line]
      : calibrationFactor * displayAdjustFactors.volts[line];
    
    return voltsDiv[line] * voltAdjustFactor;
  }
  ...
  const CHANNEL_COLORS = {
    1: '#42B983CC', // 蓝色
    2: '#FF5722', // 橙色
    OVERLAY: 'rgba(66, 185, 131, 0.8)' // 叠加波形使用绿色
  };
  ...
  // 计算中心Y坐标（抽取重复逻辑）
  function calculateCenterY(displayMode, line, canvasHeight, verticalOffset) {
    if (displayMode === 'overlay' || displayMode === 'independent') {
      // 同向叠加模式或独立显示模式：使用相同的中心线
      return canvasHeight / 2 + verticalOffset;
    } else if (displayMode === 'vertical') {
      // 垂直叠加模式：将波形分上下两部分显示
      return (line === 1) ? (canvasHeight / 4 + verticalOffset) : (3 * canvasHeight / 4 + verticalOffset);
    } else {
      // 默认情况
      return canvasHeight / 2 + verticalOffset;
    }
